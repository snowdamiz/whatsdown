...
The supported security-test target is the current `main` revision. Classical, experimental hybrid (`0x0002`), and custom group (`0x0003`) code is reachable. Release readiness requires successful internal verification of the exact Morse and pinned Mesh commits, applicable platform evidence, and resolution or exclusion of known security failures. Outside review is additional scrutiny, not a prerequisite; no independent audit is claimed. See [the security plan](mesh-private-messenger-security-plan.md).

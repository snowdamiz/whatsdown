...
test('native encrypted delivery and witnesses work with polling disabled and durable Cloudflare wakeups', {
  ...
  try {
    ...
    const modules = await Promise.all(['jobs', 'checkpoint', 'storage', 'witness'].map(async name => ({ type: 'ESModule', path: `${name}.mjs`, contents: await readFile(new URL(`./${name}.mjs`, import.meta.url), 'utf8') })));
    ...
    const privateFlow = await run(process.env.MESHC, ['test', fileURLToPath(new URL('../../tests/m13-live', import.meta.url))], { env: { ...env,
    ...
    const groupId = /PRIVACY_GROUP_ID=([a-f0-9]{64})/.exec(privateFlow.stdout)?.[1];
    assert.ok(groupId, 'native group flow must produce a synthetic marker');
    const captures = { deliveryDatabase: (await run('pg_dump', ['--data-only', '--no-owner'], { env: postgresEnv(database.href), encoding: 'buffer', maxBuffer: 32 * 1024 * 1024 })).stdout };
    for (const name of await readdir(directory)) {
      if (/\.(?:log|db)(?:-wal|-shm|-journal)?$/.test(name)) captures[name] = await readFile(join(directory, name));
    }
    assertPrivateMarkersAbsent(captures, { directBody: 'm13 live private suite-2 message',

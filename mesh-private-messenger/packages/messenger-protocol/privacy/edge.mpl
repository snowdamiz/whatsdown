...
pub fn verify_submission(input :: Bytes, now :: U64, maximum_future :: U64, difficulty :: Int) -> Bool!String do
  ...
  decode_sealed_delivery(sealed.value)?
  if difficulty < 1 || difficulty > 24 do
    Ok(false)
  else
...
pub fn encode_privacy_submission(value :: PrivacySubmission) -> Bytes!String do
  join([
      byte(1)?,
      Bytes.from_utf8("PRV"),
      write_u64(value.token.expires_at)?,
      write_u32(value.token.nonce)?,
      vector(encode_sealed_delivery(value.sealed)?)?
    ],
    0,
    Bytes.empty())
end
...
pub fn encode_stamped_request(stamp :: RequestStamp, payload :: Bytes) -> Bytes!String do
  join([
      byte(1)?,
      Bytes.from_utf8("PWR"),
      write_u64(stamp.expires_at)?,
      write_u32(stamp.nonce)?,
      vector(payload)?
    ],
    0,
    Bytes.empty())

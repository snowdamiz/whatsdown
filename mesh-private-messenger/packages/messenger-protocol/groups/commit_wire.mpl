...
fn proposal_bytes(value :: GroupProposal) -> Bytes!GroupError do
  case value do
    AddMember(leaf_index, member) -> group_join([
        group_byte(1)?,
        group_write_u16(leaf_index)?,
        group_vector(case encode_member(member) do
...
fn commit_context(version :: Int,
  ...
  group_join([
      ...
      group_byte(version)?,
      group_write_u16(suite)?,
      group_id,
      group_write_u64(prior_epoch)?,
      group_write_u64(epoch)?,
...
pub fn group_parent_bytes(value :: TreeKemParentNode) -> Bytes!GroupError do
  group_join([
      group_write_u16(value.node_index)?,
      value.public_key.bytes,
      group_byte(List.length(value.unmerged_leaves))?,
      unmerged_bytes(value.unmerged_leaves, 0, Bytes.empty())?
    ],
    0,
    Bytes.empty())
end
...
fn update_ciphertexts_bytes(values :: List<TreeKemUpdateNode>, index :: Int, output :: Bytes) -> Bytes!GroupError do
  if index >= List.length(values) do
    ...
    update_ciphertexts_bytes(values,
      ...
      group_join([
          output,
          group_byte(List.length(value.ciphertexts))?,
          ciphertext_bytes(value.ciphertexts, 0, Bytes.empty())?
        ],
        0,
        Bytes.empty())?)
  end
...
pub fn group_update_path_context(version :: Int,
  ...
  group_join([
      ...
      group_byte(List.length(update_path.nodes))?,
      update_public_bytes(update_path.nodes, 0, Bytes.empty())?
    ],
    0,
    Bytes.empty())
end

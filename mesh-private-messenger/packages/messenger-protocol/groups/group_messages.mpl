...
secret :: borrow SecretBytes) -> GroupMessage ! GroupError do
  let material = case Crypto.hkdf_sha256(secret, metadata.group_id, info, 32) do
    Err( error) -> Err(CryptoFailure(error))
    Ok( value) -> Ok(value)
  end ?
...
fn open_epoch_message(state :: borrow GroupState, message :: GroupMessage, caller_data :: Bytes) -> Bytes ! GroupError do
  ...
  if message.generation <= public.last_generation do
    return Err(Replay)
  end
  ...
  let material = case Crypto.hkdf_sha256(state.key_material.epoch_secret, public.group_id, info, 32) do
    Err( error) -> Err(CryptoFailure(error))
    Ok( value) -> Ok(value)
  end ?
  let key = message_key(material) ?

...
policy :: GroupTransparencyPolicy) -> Bytes ! GroupError do
  Ok(Crypto.sha256(group_join([Bytes.from_utf8("mesh-mls/v1/group"), group_id, tree_hash(tree), group_extensions_bytes(extensions) ?, group_policy_bytes(policy) ?],
  0,
  Bytes.empty()) ?))
end
        ...
        Ok( public_key) -> if !Bytes.secure_equals(public_key.bytes, creator.leaf_public_key.bytes) do
          ...
          if creator_leaf != 0 do
            ...
            Ok(GroupState {
              ...
              key_material : group_base_key_material(secret, leaf_private_key) ?,
              local_leaf : 0,
              next_generation : 0,
...
member :: GroupMember) -> PreparedGroupAdd ! GroupError do
  ...
  let context = group_update_path_context(1,
  3,
  state.group_id,
  state.epoch,
  epoch,
  ...
  let secret = group_next_epoch_secret(generated.secret5, context) ?
  ...
  Ok(PreparedGroupAdd {
    tree : next_tree,
    key_material : group_finish_generated(generated, secret),
    commit : commit,
    welcome : welcome,
...
leaf_public_key :: X25519PublicKey) -> PreparedJoin ! GroupError do
  ...
  if welcome.commit.version != 1 || welcome.commit.suite != 3 || Bytes.length(welcome.commit.group_id) != 32 || !group_valid_extensions(welcome.extensions,
  0,
  0) do
    Err(InvalidGroup)
  else
      ...
      case prepare_join(welcome, init_private_key, leaf_public_key) do
        ...
        Ok( prepared) -> do
          ...
          Ok(GroupState {
            ...
            key_material : group_merge_key_material(base, patch),
          ...
          prepared.context) do
            ...
            Ok( patch) -> do
              ...
              CommitApplied(GroupState {
                ...
                key_material : group_merge_key_material(state.key_material, patch),

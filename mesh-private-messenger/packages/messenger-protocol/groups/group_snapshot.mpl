...
snapshot_version :: U64) -> Bytes ! GroupError do
  ...
  group_join([group_byte(1) ?, Bytes.from_utf8("GST"), group_byte(state.version) ?, group_write_u16(state.suite) ?, state.group_id, group_write_u64(state.epoch) ?, group_write_u64(snapshot_version) ?, state.tree_hash_cache, state.transcript_hash, group_write_u16(state.local_leaf) ?, group_write_u32(state.next_generation) ?, group_byte(List.length(members)) ?, group_encode_members(members,
  0,
  Bytes.empty()) ?, group_byte(List.length(parents)) ?, group_encode_parents(parents,
  0,
  Bytes.empty()) ?, group_byte(List.length(state.received_generations)) ?, encode_generations(state.received_generations,
  0,
  Bytes.empty()) ?, group_extensions_bytes(state.extensions) ?, group_byte(List.length(state.key_material.available_levels)) ?, encode_levels(state.key_material.available_levels,
  0,
  Bytes.empty()) ?, group_policy_bytes(state.policy) ?],
  0,
  Bytes.empty())
end
...
snapshot_version :: U64) -> Bytes ! GroupError do
  if Bytes.length(account_id) != 32 || Bytes.length(device_id) != 16 || Bytes.length(group_id) != 32 || (purpose != 16 && purpose != 17) || (purpose == 16 && key_slot != 0) || (purpose == 17 && (key_slot < 1 || key_slot > 7)) do
    ...
    let object = Crypto.sha256(group_join([Bytes.from_utf8("mesh-msg/v1/group-snapshot-object"), header, group_write_u16(purpose) ?, group_byte(key_slot) ?],
    0,
    Bytes.empty()) ?)
    group_join([group_byte(1) ?, account_id, device_id, group_id, object, group_write_u16(purpose) ?, group_write_u64(snapshot_version) ?],
    0,
    Bytes.empty())
  end
end
...
snapshot_version :: U64) -> Bytes ! GroupError do
  ...
  if Bytes.length(header) > 64711 || Bytes.length(sealed) != 99 || Bytes.length(leaf_private) != 99 || Bytes.length(level0) != 99 || Bytes.length(level1) != 99 || Bytes.length(level2) != 99 || Bytes.length(level3) != 99 || Bytes.length(level4) != 99 || Bytes.length(level5) != 99 do
    ...
    group_join([header, group_vector(sealed) ?, group_vector(leaf_private) ?, group_vector(level0) ?, group_vector(level1) ?, group_vector(level2) ?, group_vector(level3) ?, group_vector(level4) ?, group_vector(level5) ?],
    0,
    Bytes.empty())
  end
end
...
fn parsed_group_header(value :: ParsedGroupSnapshot) -> Bytes ! GroupError do
  group_join([group_byte(1) ?, Bytes.from_utf8("GST"), group_byte(value.version) ?, group_write_u16(value.suite) ?, value.group_id, group_write_u64(value.epoch) ?, group_write_u64(value.snapshot_version) ?, value.tree_hash, value.transcript_hash, group_write_u16(value.local_leaf) ?, group_write_u32(value.next_generation) ?, group_byte(List.length(value.members)) ?, group_encode_members(value.members,
  0,
  Bytes.empty()) ?, group_byte(List.length(value.parent_nodes)) ?, group_encode_parents(value.parent_nodes,
  0,
  Bytes.empty()) ?, group_byte(List.length(value.received_generations)) ?, encode_generations(value.received_generations,
  0,
  Bytes.empty()) ?, group_extensions_bytes(value.extensions) ?, group_byte(List.length(value.available_levels)) ?, encode_levels(value.available_levels,
  0,
  Bytes.empty()) ?, group_policy_bytes(value.policy) ?],
  0,
  Bytes.empty())
end

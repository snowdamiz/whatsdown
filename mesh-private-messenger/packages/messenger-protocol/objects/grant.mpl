...
fn work_hash(value :: ObjectGrantRequest, nonce :: Int) -> Bytes ! String do
  Ok(Crypto.sha256(join([Bytes.from_utf8("mesh-msg/v1/object-grant-work"), value.object_id, write_u64(value.work_expires_at) ?, write_u32(nonce) ?, write_u32(value.part_count) ?, write_u64(value.expires_at) ?, Crypto.sha256(value.upload_capability), Crypto.sha256(value.download_capability)],
  0,
  Bytes.empty()) ?))
end
...
pub fn encode_grant(value :: ObjectGrantRequest) -> Bytes ! String do
  if !valid_shape(value) || value.nonce < 0 do
    ...
    join([byte(1) ?, Bytes.from_utf8("OGR"), value.object_id, write_u32(value.part_count) ?, write_u64(value.expires_at) ?, write_u64(value.work_expires_at) ?, write_u32(value.nonce) ?, value.upload_capability, value.download_capability],
    0,
    Bytes.empty())
  end
end

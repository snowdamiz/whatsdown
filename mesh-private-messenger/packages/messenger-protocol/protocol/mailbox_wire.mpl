...
fn mailbox_fetch_content(value :: MailboxFetch) -> Bytes ! ProtocolError do
  if value.version != 2 do
    ...
    if Bytes.length(value.mailbox_token_hash) != 32 do
      ...
      protocol_join([protocol_byte(value.version) ?, Bytes.from_utf8("FET"), value.mailbox_token_hash, protocol_write_u64(value.after_sequence) ?, protocol_write_u64(value.issued_at) ?],
      0,
      Bytes.empty())
    end
  end
...
fn mailbox_ack_content(value :: MailboxAck) -> Bytes ! ProtocolError do
  if value.version != 2 do
    ...
    if Bytes.length(value.mailbox_token_hash) != 32 do
      ...
      0,
      protocol_join([protocol_byte(value.version) ?, Bytes.from_utf8("ACK"), value.mailbox_token_hash, protocol_write_u64(value.issued_at) ?, protocol_byte(List.length(value.envelope_ids)) ?],
      0,
      Bytes.empty()) ?)
    end
  end

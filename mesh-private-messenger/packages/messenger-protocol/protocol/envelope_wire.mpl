...
from Protocol.V1 import InnerEnvelope, OuterEnvelope, ProtocolError, protocol_supported_suite
...
pub fn encode_inner_envelope(value :: InnerEnvelope) -> Bytes ! ProtocolError do
  ...
  let encoded = protocol_join([protocol_byte(value.version) ?, Bytes.from_utf8("PAY"), value.sender_account_id, value.sender_device_id, value.recipient_device_id, value.conversation_id, value.client_message_id, protocol_write_u64(value.client_timestamp) ?, protocol_write_u16(value.message_type) ?, protocol_vector(value.body) ?, protocol_vector(value.reply_reference) ?, protocol_vector(value.attachment_manifest) ?, protocol_byte(value.receipt_policy) ?, protocol_write_length(value.disappearing_seconds) ?, protocol_encode_extensions(value.extensions) ?],
  0,
  Bytes.empty()) ?
...
fn supported_outer_suite(value :: Int) -> Bool do
  protocol_supported_suite(value) || value == 3
end
...
pub fn encode_outer_envelope(value :: OuterEnvelope) -> Bytes ! ProtocolError do
  ...
  protocol_join([protocol_byte(value.version) ?, Bytes.from_utf8("MSG"), value.envelope_id, value.mailbox_token, protocol_write_u16(value.suite) ?, protocol_write_u64(value.expiration) ?, protocol_write_length(value.padding_bucket) ?, protocol_vector(value.ciphertext) ?],
  0,
  Bytes.empty())
end

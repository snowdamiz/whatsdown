...
pub fn encode_directory_lookup(username :: String) -> Bytes ! ProtocolError do
  protocol_join([protocol_byte(1) ?, Bytes.from_utf8("DLK"), protocol_vector(encode_username(username) ?) ?],
  0,
  Bytes.empty())
end
...
pub fn encode_directory_entry(value :: DirectoryEntry) -> Bytes ! ProtocolError do
  ...
  protocol_join([protocol_byte(value.version) ?, Bytes.from_utf8("DRE"), protocol_vector(encode_username(value.username) ?) ?, protocol_vector(value.account_identity) ?, protocol_vector(value.prekey_bundle) ?, value.mailbox_token],
  0,
  Bytes.empty())
end
...
pub fn encode_device_link_request(value :: DeviceLinkRequest) -> Bytes ! ProtocolError do
  ...
  protocol_join([protocol_byte(value.version) ?, Bytes.from_utf8("LNK"), suite, value.nonce, value.device_id, value.signing_public_key, value.dh_public_key, value.post_quantum_public_key, protocol_write_u64(value.capabilities) ?, protocol_write_u64(value.created_at) ?, protocol_write_u64(value.expires_at) ?],
  0,
  Bytes.empty())
end
...
pub fn encode_device_link_authorization(value :: DeviceLinkAuthorization) -> Bytes ! ProtocolError do
  ...
  protocol_join([protocol_byte(value.version) ?, Bytes.from_utf8("LNA"), value.request_hash, protocol_vector(encode_username(value.username) ?) ?, protocol_vector(value.account_identity) ?, protocol_vector(value.device_credential) ?, value.authorization_signature],
  0,
  Bytes.empty())
end
...
pub fn encode_device_set(value :: DeviceSet) -> Bytes ! ProtocolError do
  ...
  let header = protocol_join([protocol_byte(value.version) ?, Bytes.from_utf8("DVS"), protocol_vector(encode_username(value.username) ?) ?, protocol_vector(value.account_identity) ?, protocol_write_u64(value.sequence) ?, protocol_byte(List.length(value.devices)) ?],
  0,
  Bytes.empty()) ?
...
pub fn encode_device_revocation(value :: DeviceRevocation) -> Bytes ! ProtocolError do
  ...
  protocol_join([protocol_byte(value.version) ?, Bytes.from_utf8("DVR"), value.account_id, value.device_id, protocol_write_u64(value.sequence) ?, value.signature],
  0,
  Bytes.empty())
end

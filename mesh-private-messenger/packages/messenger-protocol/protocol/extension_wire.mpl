...
pub fn protocol_encode_suites(values :: List<Int>) -> Bytes!ProtocolError do
  ...
  protocol_join([
      protocol_byte(List.length(values))?,
      encode_suite_entries(values, 0, Bytes.empty())?
    ],
    0,
    Bytes.empty())
end
...
fn encode_extension_entries(values :: List<ProtocolExtension>, index :: Int, output :: Bytes) -> Bytes!ProtocolError do
  if index >= List.length(values) do
    ...
    let encoded = protocol_join([
        protocol_write_u16(extension.id)?,
        protocol_byte(0)?,
        protocol_vector(extension.value)?
      ],
      0,
      Bytes.empty())?
...
pub fn protocol_encode_extensions(values :: List<ProtocolExtension>) -> Bytes!ProtocolError do
  ...
  protocol_join([
      protocol_write_u16(List.length(values))?,
      encode_extension_entries(values, 0, Bytes.empty())?
    ],
    0,
    Bytes.empty())
end

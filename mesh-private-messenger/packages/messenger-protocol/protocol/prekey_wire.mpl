...
pub fn encode_prekey_bundle(value :: PrekeyBundle) -> Bytes ! ProtocolError do
  ...
  protocol_join([protocol_byte(value.version) ?, Bytes.from_utf8("PKB"), protocol_write_u16(value.suite) ?, protocol_vector(value.device_credential) ?, value.identity_dh_public_key, value.signing_public_key, protocol_write_u64(value.signed_prekey_id) ?, value.signed_prekey, value.signed_prekey_signature, protocol_write_u64(value.one_time_prekey_id) ?, protocol_vector(value.one_time_prekey) ?, value.post_quantum_prekey, protocol_encode_suites(value.supported_suites) ?, protocol_write_u64(value.expires_at) ?, protocol_encode_extensions(value.extensions) ?],
  0,
  Bytes.empty())
end

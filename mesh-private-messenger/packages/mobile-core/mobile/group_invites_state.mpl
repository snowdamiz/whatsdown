...
fn encode_record(value :: GroupInvitation) -> Bytes ! String do
  encode_output_list([value.id, value.group_id, value.inviter_account, value.inviter_device, value.recipient_account, value.recipient_device, Bytes.from_utf8(value.username), value.baseline, mobile_write_u64(value.expires_at) ?, mobile_byte(value.state) ?, value.key_package, value.inviter_signing_key])
end
...
pub fn load_invitations(path :: String, key :: borrow StorageKey) -> List < GroupInvitation > ! String do
  ...
  if List.length(live) != List.length(records) do
    ...
    fn (value) do [invitation_key_label(value.id, "package"), invitation_key_label(value.id, "init"), invitation_key_label(value.id,
    "leaf")] end)
...
fn validate_control(inner :: InnerEnvelope) -> Bool ! String do
  ...
  if inner.message_type == 3 do
    let _ = mobile_read_u64(List.get(parts, 2)) ?
    Ok(Bytes.length(List.get(parts, 3)) == 188)
  else
    let _ = decode_group_key_package(List.get(parts, 2)) ?
    Ok(true)
  end
end

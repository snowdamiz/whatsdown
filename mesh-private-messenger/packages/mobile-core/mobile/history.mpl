from Mobile.Presentation import consume_presented_message
...
direction :: Int) -> Result <( String, Bytes), String > do
  ...
  let body = consume_presented_message(database_path, wrapping_key, inner.sender_account_id, Bytes.empty(), inner.body) ?
  let updated = if Bytes.length(body) == 0 do entries else List.append(entries,
  MobileHistoryEntry {
    direction : direction,
...
values :: List < Bytes >) -> List < Bytes > ! String do
  if index >= List.length(session_ids) do
    ...
    0) do
      ...
      session_ids,
      0) ?
      let has_history = case load_blob(database_path, history_label(preferred.record.conversation_id)) do
        Ok(_) -> Ok(true)
        Err(error) -> if error == "local_state_not_found" do Ok(false) else Err(error) end
      end ?
      collect_conversations(database_path,
      wrapping_key,
      local_account_id,
      ...
      if has_history do List.append(values, conversation_summary(preferred) ?) else values end)
    end
  end
end

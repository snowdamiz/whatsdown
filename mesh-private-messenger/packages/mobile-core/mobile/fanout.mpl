...
from Mobile.Codec import current_time, encode_output_list, outer_bytes, random_bytes
...
from Session.Ratchet import RatchetError, RatchetMessage, encrypt
...
pub fn send_fanout(request :: MobileFanoutRequest) -> Bytes ! String do
  send_fanout_control(% { request | body : present_message(request.database_path, Bytes.empty(), request.body) ? }, 1, [], [])
end

pub fn send_fanout_control(request :: MobileFanoutRequest, message_type :: Int,
extra_labels :: List<String>, extra_blobs :: List<Bytes>) -> Bytes ! String do
  ...
  if invalid_fanout_sets(local, peers, local_devices) do
    ...
    List.new(),
    0) ?
    let claimed_prekeys = if message_type != 1 do Ok(peer_prekeys) else append_needed_prekeys(request.database_path,
    wrapping_key,
    session_ids,
    ...
    let added_count = List.length(peers.profiles) + if message_type == 1 do List.length(local_devices.profiles) - 1 else 0 end
    if List.length(pending_ids) + added_count > 64 do
      ...
      request.body,
      message_type,
      if message_type == 1 do anchor.disappearing_seconds else 0 end,
      0,
      List.new()) ?
      ...
      let (history_keys, history_blobs) = if message_type != 1 do Ok((extra_labels, extra_blobs)) else
      let ( history_key, history_blob) = updated_history(request.database_path,
      wrapping_key,
      history_inner,
      1) ?
      Ok(([history_key], [history_blob]))
      end ?
      let sync_body = encode_sync_payload(local, List.head(peers.profiles), history_inner) ?
      let prepared = if message_type != 1 do Ok(prepared_peers) else self_fanout(request.database_path,
      wrapping_key,
      session_ids,
      claimed_prekeys,
      local_device,

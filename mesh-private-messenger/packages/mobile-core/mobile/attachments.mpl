...
pub fn encode_reference(value :: MobileAttachmentReference) -> Bytes ! String do
  if !valid_reference(value) do
    ...
    mobile_join([reference_header() ?, value.object_id, value.download_capability, mobile_vector(value.encrypted_manifest) ?, mobile_vector(value.wrapped_key) ?],
    0,
    Bytes.empty())
  end
end
...
recipients :: List < MobileAttachmentRecipient >) -> Bytes ! String do
  if Bytes.length(encoded) == 0 do
    ...
    mobile_join([group_header() ?, value.object_id, value.download_capability, mobile_vector(value.encrypted_manifest) ?, mobile_write_u32(List.length(recipients)) ?, entries],
    0,
    Bytes.empty())
  end
end
...
output :: List < Bytes >) -> Bytes ! String do
  if index >= List.length(parts) do
    ...
    let summary = encode_output_list([part, value.object_id, value.download_capability, manifest.filename, manifest.mime_type, mobile_write_u32(manifest.plaintext_size) ?, mobile_write_u32(manifest.chunk_count) ?, mobile_write_u32(manifest.chunk_size) ?, mobile_write_u64(manifest.expires_at) ?]) ?
...
pub fn prepare_attachment(request :: MobileAttachmentPrepareRequest) -> Bytes ! String do
  if request.plaintext_size <= 0 || request.plaintext_size > maximum_attachment_size() do
    ...
    encode_output_list([reference, object_id, upload_capability, grant, encode_complete(control) ?, encode_delete(control) ?, encrypted_manifest])
  end
end

...
# Register, resolve and prekey claim are anonymous, so the directory asks for
# proof of work minted for that endpoint and those exact bytes. The stamp is
# good for four minutes: the directory accepts five, which leaves a minute for
# this device's clock running ahead of the server's.

pub fn stamped_request(label :: String, payload :: Bytes) -> Bytes!String do
  let config = native_security_config()?
  let expires_at = U64.add(current_time()?, mobile_wide("240000")?)?
  encode_stamped_request(mint_request_stamp(label, payload, expires_at, config.abuse_difficulty)?,
    payload)
end
...
fn parse_expo_raw_token_inner(input :: Bytes) -> MobileExpoRawToken!String do
  ...
  case finish(device_token.state) do
    Err(_) -> Err("invalid")
    Ok(_) -> Ok(nil)
  end?

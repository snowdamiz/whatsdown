...
fn encode_group_history_entry(value :: MobileGroupHistoryEntry) -> Bytes ! String do
  if !valid_group_history_entry(value) do
    ...
    encode_output_list([mobile_byte(1) ?, mobile_byte(value.direction) ?, mobile_write_u64(value.epoch) ?, value.sender_account_id, value.sender_device_id, mobile_write_u64(value.timestamp) ?, value.body, value.attachment, value.message_id])
  end
end
...
local_leaf :: Int) -> Bytes ! String do
  ...
  encode_output_list([mobile_byte(1) ?, mobile_write_u32(value.leaf_index) ?, mobile_byte(if value.leaf_index == local_leaf do
    ...
    0
  end) ?, value.member.account_id, value.member.device_id, mobile_write_u64(value.member.directory_sequence) ?, mobile_byte(value.member.witness_count) ?, username])
end

...
entry :: MobileGroupHistoryEntry) -> Bytes ! String do
  let label = group_history_label(group_id)
  let body = consume_presented_message(database_path, wrapping_key, entry.sender_account_id, group_id, entry.body) ?
  let previous = load_group_history(database_path, wrapping_key, group_id) ?
  let entries = if Bytes.length(body) == 0 do previous else List.append(previous, % { entry | body : body }) end
...
pub fn group_join_scoped_label(scope :: String, kind :: String) -> String do
  if scope == "" do group_join_label(kind) else "group-invitation/#{scope}/#{kind}/v1" end
end
...
pub fn group_target_envelopes(targets :: List < GroupDeliveryTarget >,
packet :: Bytes,
now :: U64,
index :: Int,
...
pub fn group_add_envelopes(targets :: List < GroupDeliveryTarget >,
recipient_leaf :: Int,
commit_packet :: Bytes,

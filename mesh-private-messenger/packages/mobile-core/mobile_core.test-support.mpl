...
pub fn group_transparency_valid_for_test(database_path :: String) -> Bool!String do
  load_transparency_view(database_path, platform_key()?)?
  Ok(true)
end
...
pub fn age_last_resort_for_test(database_path :: String, milliseconds :: Int) -> Result<(), String> do
  ...
  if Bytes.length(current) != 48 do
    ...
    store_updated_blobs(database_path,
      ...
      [
        seal_local(aged, wrapping_key, local_context("last-resort-prekey/v1")?)?,
        seal_local(aged_retired(retired, 0, milliseconds, Bytes.empty())?,
          wrapping_key,

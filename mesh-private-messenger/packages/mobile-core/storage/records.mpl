...
pub fn store_record_changes(database_path :: String, labels :: List<String>,
blobs :: List<Bytes>, removed :: List<String>) -> Result<(), String> do
  with_record_transaction(database_path, fn(database) do
...
prekey_active_blob :: Bytes) -> Result <(), String > do
  ...
  fn (database) do
    ...
    delete_blobs(database, removed_labels, 0) ?
    put_blob(database, "one-time-prekeys/v1", prekey_index_blob) ?
    put_blob(database, "one-time-prekey-active/v1", prekey_active_blob)
  end)
end
...
history_blob :: Bytes) -> Result <(), String > do
  ...
  fn (database) do
    ...
    put_blob(database, history_key, history_blob)
  end)
end

...
fn proof() -> Bool!String do
  ...
  case authorize_device_link_for_set_export(request([
    Bytes.from_utf8(root_path),
    root_set,
    request_wire
  ])?) do
  ...
  assert(install_group_transparency_for_test(root_path,
    ...
    root_set)?)
  let authorization_wire = authorize_device_link_for_set_export(request([
    Bytes.from_utf8(root_path),
    root_set,
    request_wire
  ])?)?
  ...
  let linked_profile = complete_device_link_export(request([
  ...
  let sender_profile = create_account_export(request([
    Bytes.from_utf8(sender_path),
    Bytes.from_utf8("bob")
  ])?)?
  let linked_message = Bytes.from_utf8("hybrid linked device")
  let linked_initial = start_conversation_export(request([
    Bytes.from_utf8(sender_path),
    linked_profile,
    linked_message
  ])?)?
  ...
  case create_device_revocation_export(request([
    Bytes.from_utf8(root_path),
    unverified_successor,
    completed_credential.device_id
  ])?) do
    Ok(_) -> assert(false)
    Err(error) -> assert(error == "device_set_transparency_unverified")
  end
  let expected_linked = inspect_output("alice",
    ...
    1,
    [
      inspect_row(root_credential.device_id, 1, 1)?,
      inspect_row(completed_credential.device_id, 1, 0)?
    ])?
  ...
  assert(install_group_transparency_for_test(root_path,
    ...
    linked_set)?)
  assert(Bytes.secure_equals(inspect_device_set_export(request([
      Bytes.from_utf8(root_path),
      linked_set
    ])?)?,
    expected_linked))
  ...
  let expected_revoked = inspect_output("alice",
    ...
    [
      inspect_row(root_credential.device_id, 1, 1)?,
      inspect_row(completed_credential.device_id, 0, 0)?
    ])?
  assert(Bytes.secure_equals(inspect_device_set_export(request([
      Bytes.from_utf8(root_path),
      revoked_set
  ...
  assert(Bytes.secure_equals(forget_on_proof_export(request([
      Bytes.from_utf8(linked_path),
      revocation_wire
    ])?)?,
    byte(2)?))

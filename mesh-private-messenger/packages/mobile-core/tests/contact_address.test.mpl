...
fn answered(path :: String) -> Result<(), String> do
  ...
  reconcile_prekeys_export(request([Bytes.from_utf8(path), answer], 0, Bytes.empty())?)?
  Ok(nil)
end
...
fn proof() -> Bool!String do
  ...
  let early = start_conversation_export(request([
      Bytes.from_utf8(alice),
      carol_profile,
      Bytes.from_utf8("hello carol")
    ],
    0,
    Bytes.empty())?)?
  settle(alice, early, true)?
  receive_initial_export(request([Bytes.from_utf8(carol), early], 0, Bytes.empty())?)?
  ...
  receive_initial_export(request([Bytes.from_utf8(bob), hello], 0, Bytes.empty())?)?
  update_conversation_export(request([Bytes.from_utf8(bob), alice_profile, byte(1)?, write_u32(0)?],
    0,
    Bytes.empty())?)?
  ...
  update_conversation_export(request([Bytes.from_utf8(alice), bob_profile, byte(2)?, write_u32(0)?],
    0,
    Bytes.empty())?)?

...
fn bob_package_for_group(accounts :: GroupAccountFixture, group_id :: Bytes) -> Bytes!String do
  ...
  case group_add_export(group_vectors([
...
fn add_bob_to_group(accounts :: GroupAccountFixture,
  ...
  let bob_welcome_output = group_add_export(group_vectors([
    Bytes.from_utf8(accounts.alice_path),
    group_id,
    accounts.bob_set,
    bob_package
  ...
  group_create_ensure(Bytes.secure_equals(group_receive_export(group_vectors([
        Bytes.from_utf8(accounts.bob_path),
        List.head(bob_welcome)
      ])?)?,
      group_id),
...
pub fn create_group_with_bob(accounts :: GroupAccountFixture) -> Bytes!String do
  ...
  install_signed_transparency(accounts.bob_path, transparency, accounts.alice_set)?
  install_signed_transparency(accounts.linked_path, transparency, accounts.bob_set)?
  Ok(group_id)
end

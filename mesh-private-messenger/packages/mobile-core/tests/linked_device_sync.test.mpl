...
fn reserve(path :: String, peer_set :: Bytes, local_set :: Bytes, claimed :: Bytes) -> Bool ! String do
  assert(Bytes.length(reserve_fanout_prekey_export(request([Bytes.from_utf8(path), peer_set, local_set, claimed]) ?) ?) == 0)
  Ok(true)
end
...
fn fanout(path :: String, peer_set :: Bytes, local_set :: Bytes, body :: String) -> List < Bytes > ! String do
  output_list(send_fanout_export(request([Bytes.from_utf8(path), peer_set, local_set, Bytes.from_utf8(body)]) ?) ?)
end
...
fn accept(path :: String, peer_profile :: Bytes) -> Bool ! String do
  assert(Bytes.secure_equals(update_conversation_export(request([Bytes.from_utf8(path), peer_profile, byte(1) ?, write_u32(0) ?]) ?) ?,
  Bytes.from_utf8("ok")))
  Ok(true)
end
...
fn proof() -> Bool ! String do
  ...
  let carol_profile = create_account_export(request([Bytes.from_utf8(carol_path), Bytes.from_utf8("carol")]) ?) ?
  let link_request = create_link_request_export(Bytes.from_utf8(linked_path)) ?
  let authorization = authorize_device_link_export(request([Bytes.from_utf8(root_path), link_request]) ?) ?
  assert(Bytes.length(complete_device_link_export(request([Bytes.from_utf8(linked_path), authorization]) ?) ?) > 0)
  ...
  let alice_view = signed_transparency_view([leaf_hash(alice_set) ?, leaf_hash(bob_set) ?, leaf_hash(carol_set) ?]) ?
  ...
  let carol_initial = start_conversation_export(request([Bytes.from_utf8(root_path), carol_profile, Bytes.from_utf8(hello_carol)]) ?) ?

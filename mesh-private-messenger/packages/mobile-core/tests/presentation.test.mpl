...
fn exercise() -> Bool ! String do
  ...
  Bytes.from_utf8("old client")))
  let updated_group = group_vectors([Bytes.from_utf8("Weekend walks"), Bytes.from_utf8("data:image/jpeg;base64,/9j/2Q=="), mobile_write_u64(mobile_wide("2") ?) ?]) ?
  ...
  profile.account_id,
  packet) ?
  assert(Bytes.secure_equals(presentation_load_export(group_vectors([Bytes.from_utf8(path), group_key]) ?) ?,
  updated_group))
  let forged = group_vectors([Bytes.from_utf8("Not the creator"), Bytes.empty(), mobile_write_u64(mobile_wide("3") ?) ?]) ?
  consume_presented_message(path,
  wrapping_key,
  repeated(8, 32) ?,
  group_id,
...
fn exercise_nicknames() -> Bool ! String do
  ...
  let nickname = group_vectors([Bytes.from_utf8("Dad"), Bytes.empty(), mobile_write_u64(mobile_wide("2") ?) ?]) ?
  ...
  assert(Bytes.secure_equals(presentation_load_export(group_vectors([Bytes.from_utf8(path), Bytes.from_utf8("user/" <> Bytes.to_hex(peer))]) ?) ?,
  shared))
  File.delete(path) ?
  Ok(true)
end

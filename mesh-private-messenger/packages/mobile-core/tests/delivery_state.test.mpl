...
fn send_many(path :: String, peer :: Bytes, remaining :: Int) -> Result <(), String > do
  if remaining <= 0 do
    ...
    let _ = send_message_export(request([Bytes.from_utf8(path), peer, Bytes.from_utf8("again")],
    0,
    Bytes.empty()) ?) ?
...
fn proof() -> Bool ! String do
  ...
  let _ = create_account_export(request([Bytes.from_utf8(alice), Bytes.from_utf8("alice")],
  0,
  Bytes.empty()) ?) ?
  ...
  let first = start_conversation_export(request([Bytes.from_utf8(alice), bob_profile, Bytes.from_utf8("hello")],
  0,
  Bytes.empty()) ?) ?
  ...
  let second = send_message_export(request([Bytes.from_utf8(alice), bob_profile, Bytes.from_utf8("still there?")],
  0,
  Bytes.empty()) ?) ?

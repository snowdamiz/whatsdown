...
fn proof() -> Bool!String do
  ...
  case prepare_fanout_prekeys_export(request([
    Bytes.from_utf8(alice_path),
    bob_set,
    alice_set,
    Bytes.from_utf8("http://127.0.0.1:18996")
  ])?) do
    Ok(_) -> assert(false)
  ...
  case reserve_fanout_prekey_export(request([
    ...
    bob_set,
    alice_set,
    bundle_wire(changed_static_bundle)?
  ])?) do
    Ok(_) -> assert(false)
    Err(error) -> assert(error == "invalid_fanout_prekeys")
  end
  assert(database_fingerprint(alice_path)? == before_failure)
  let reserve_request = request([
    Bytes.from_utf8(alice_path),
    bob_set,
    alice_set,
    bundle_wire(claimed_linked_bundle)?
  ])?
  ...
  case receive_message_export(request([
    Bytes.from_utf8(alice_path),
    test_sealed_tamper_envelope(root_reply)?
  ])?) do
    Ok(_) -> assert(false)
  ...
  case receive_message_export(request([
    Bytes.from_utf8(alice_path),
    test_ratchet_jump_envelope(alice_path, root_reply)?
  ])?) do
    Ok(_) -> assert(false)
  ...
  case receive_message_export(request([
    Bytes.from_utf8(alice_path),
    test_ratchet_tamper_envelope(alice_path, root_reply)?
  ])?) do
    Ok(_) -> assert(false)
    Err(error) -> assert(error == "message_rejected")
  end
  ...
  assert(Bytes.secure_equals(safety,
    safety_number_export(request([Bytes.from_utf8(linked_path), bob_profile])?)?))
  let linked_history = output_list(load_history_export(request([
    Bytes.from_utf8(linked_path),
    bob_profile
  ])?)?)?

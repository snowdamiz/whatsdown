...
pub fn exercise_group_removal(accounts :: GroupAccountFixture, group_id :: Bytes) -> Bool!String do
  ...
  let removal_output = group_remove_export(group_vectors([
  ...
  group_remove_ensure(Bytes.secure_equals(group_receive_export(group_vectors([
  ...
  let after_output = group_send_export(group_vectors([
    Bytes.from_utf8(accounts.alice_path),
    group_id,
    after_removal
  ])?)?
  ...
  let final_inspect = output_list(group_inspect_export(group_vectors([
    Bytes.from_utf8(accounts.alice_path),
    group_id
  ])?)?)?

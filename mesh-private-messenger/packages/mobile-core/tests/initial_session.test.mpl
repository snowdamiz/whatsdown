...
fn proof() -> Bool!String do
  ...
  let bob_profile_id2 = create_account_export(request([
  ...
  let malformed_ack = ack(process_delivery_batch_export(request([
    Bytes.from_utf8(bob_path),
    delivery_batch(malformed_outer)?
  ])?)?)?
  ...
  let sealed_malformed_ack = ack(process_delivery_batch_export(request([
    Bytes.from_utf8(bob_path),
    delivery_batch(sealed_malformed_outer)?
  ])?)?)?
  ...
  let received_ack = ack(process_delivery_batch_export(request([
    Bytes.from_utf8(bob_path),
    initial_batch
  ])?)?)?
  ...
  assert(database_fingerprint(bob_path)? != before_failure)
  let replay_ack = ack(process_delivery_batch_export(request([
    Bytes.from_utf8(bob_path),
    initial_batch
  ])?)?)?

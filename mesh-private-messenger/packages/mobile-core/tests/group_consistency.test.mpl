...
fn proof() -> Bool!String do
  ...
  let bob_cache = evidence_bytes(bob.device_set,
    ...
    witness_a_pair.private_key,
    witness_b_pair.private_key)?
  verify_for(bob, "bob", bob_cache, service_public_key, witness_a_public_key, witness_b_public_key)?
  ...
  let welcome_output = group_add_export(request([
    Bytes.from_utf8(alice.path),
    group_id,
    bob.device_set,
    bob_package
  ])?)?

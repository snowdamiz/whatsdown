...
fn send_until_refresh(accounts :: GroupAccountFixture, group_id :: Bytes, remaining :: Int) -> Result <(), String > do
  if remaining == 0 do
    ...
    let entries = output_list(group_send_export(group_vectors([Bytes.from_utf8(accounts.alice_path), group_id, Bytes.empty()]) ?) ?) ?
...
fn proof() -> Bool ! String do
  ...
  let entries = output_list(group_send_export(group_vectors([Bytes.from_utf8(accounts.alice_path), group_id, plaintext]) ?) ?) ?
  ...
  let late = process_delivery_batch_export(group_vectors([Bytes.from_utf8(accounts.bob_path), delivery_batch(List.get(entries,
  1)) ?]) ?) ?
  assert(Bytes.length(late) == 0)
  group_receive_export(group_vectors([Bytes.from_utf8(accounts.bob_path), List.head(entries)]) ?) ?
  let opened = group_receive_export(group_vectors([Bytes.from_utf8(accounts.bob_path), List.get(entries,
  1)]) ?) ?

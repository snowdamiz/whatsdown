...
fn proof(malformed_first :: Bool) -> Bool ! String do
  ...
  if malformed_first do
    let malformed = output_list(send_fanout_control(MobileFanoutRequest {
      ...
      body : encode_output_list([repeated(7, 16) ?, repeated(9, 32) ?, Bytes.from_utf8("x"), repeated(5,
      188) ?]) ?
    },
    3,
    [],
    []) ?) ?
    let _ = receive_initial_export(group_vectors([bob_path, List.head(malformed)]) ?) ?
  ...
  let acceptance = output_list(group_invitation_accept_export(group_vectors([bob_path, alice_set, bob_set, reference]) ?) ?) ?
  assert(List.length(acceptance) == 1)
  let _ = receive_message_export(group_vectors([alice_path, List.head(acceptance)]) ?) ?
  ...
  let _ = group_key_package_export(bob_path) ?
  ...
  let packet = decode_group_packet(outer(List.head(welcome)) ?.ciphertext) ?
  ...
  let _ = receive_message_export(group_vectors([bob_path, List.head(next)]) ?) ?

...
fn proof() -> Bool ! String do
  ...
  0,
  Bytes.empty()) ?) ?
  let _ = create_account_export(request([Bytes.from_utf8(bob), Bytes.from_utf8("bob")],
  0,
  Bytes.empty()) ?) ?
  let hello = start_conversation_export(request([Bytes.from_utf8(bob), alice_profile, Bytes.from_utf8("hello")],
  0,
  Bytes.empty()) ?) ?
  let _ = outbox_ack_export(request([Bytes.from_utf8(bob), hello], 0, Bytes.empty()) ?) ?
  let late = send_message_export(request([Bytes.from_utf8(bob), alice_profile, Bytes.from_utf8("second")],
  0,
  Bytes.empty()) ?) ?
  ...
  let third = send_message_export(request([Bytes.from_utf8(bob), alice_profile, Bytes.from_utf8("third")],
  0,
  Bytes.empty()) ?) ?

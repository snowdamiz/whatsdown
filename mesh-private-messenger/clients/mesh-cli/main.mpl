...
fn run_device_a() -> Int!String do
  ...
  submit_envelope(initial_outer, 202)?
  submit_envelope(initial_outer, 200)?
  ...
  let second_outer = outbound(directory.mailbox_token,
    encode_packet(RatchetPacket(ratchet_wire(second)?))?,
    bob_credential.dh_public_key)?
  submit_envelope(third_outer, 202)?
  submit_envelope(first_outer, 202)?
  submit_envelope(second_outer, 202)?
  submit_envelope(duplicate_outer, 202)?

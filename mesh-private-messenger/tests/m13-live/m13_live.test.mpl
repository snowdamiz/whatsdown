...
fn verified_set(base_url :: String,
  ...
  let lookup = resolve_request_export(request([
    Bytes.from_utf8(database_path),
...
fn proof() -> Bool!String do
  ...
  if String.length(core_url) == 0 || String.length(edge_url) == 0 || String.length(alice_path) == 0 || String.length(bob_path) == 0 || String.length(proof_plaintext) == 0 do
    ...
    assert(Bytes.length(prepare_fanout_prekeys_export(request([
      Bytes.from_utf8(alice_path),
      bob_set,
      alice_set,
      Bytes.from_utf8(core_url)
    ])?)?) == 0)
    ...
    verified_set(core_url, bob_path, "bob", 0)?
    verified_set(core_url, bob_path, "alice", 2)?
    ...
    presentation_save_export(request([
      Bytes.from_utf8(alice_path),
      Bytes.from_utf8("group/" <> Bytes.to_hex(group_id)),
      request([group_name, Bytes.empty()])?
    ])?)?
    assert(submit_and_receive_group(core_url,
      edge_url,
      bob_path,
      group_send_export(request([
        Bytes.from_utf8(alice_path),
        group_id,
        Bytes.from_utf8("private-group-body-373bc740b242")
      ])?)?)?)
    assert(List.length(output_list(group_history_export(request([
      Bytes.from_utf8(bob_path),
      group_id
    ])?)?)?) == 1)

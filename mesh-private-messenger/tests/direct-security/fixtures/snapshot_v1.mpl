...
fn encode_header_v1(state :: borrow RatchetState, snapshot_version :: U64) -> Bytes!SnapshotErrorV1 do
  ...
  if !valid do
    ...
    join_v1([
        byte_v1(1)?,
        Bytes.from_utf8("RST"),
        write_u16_v1(state.suite)?,
        state.session_id,
        write_u64_v1(snapshot_version)?,
        state.local_ratchet_public.bytes,
        state.remote_ratchet_public.bytes,
        write_u32_v1(state.previous_chain_length)?,
        write_u32_v1(state.sent_count)?,
        write_u32_v1(state.received_count)?,
        byte_v1(pending)?
      ],
      0,
      Bytes.empty())
...
fn storage_context_v1(account_id :: Bytes,
  ...
  if Bytes.length(account_id) != 32 || Bytes.length(device_id) != 16 || Bytes.length(session_id) != 32 || !supported do
    ...
    join_v1([
        ...
        device_id,
        session_id,
        storage_object_v1(header, purpose)?,
        write_u16_v1(purpose)?,
        write_u64_v1(snapshot_version)?
      ],
      0,
...
fn seal_snapshot_v1(state :: borrow RatchetState,
  ...
  join_v1([
      header,
      vector_v1(root_key)?,
      vector_v1(sending_chain_key)?,
      vector_v1(receiving_chain_key)?,
      vector_v1(local_ratchet_private)?,
      vector_v1(skipped_keys)?
    ],
    0,
    Bytes.empty())
end

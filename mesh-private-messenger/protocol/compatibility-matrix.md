...
|---|---|---|---|---|
| Classical one-to-one | protocol `1`, suite `0x0001` | ratchet snapshot `1` | Authenticate the selected suite; reject unknown mandatory extensions and downgrade below the strongest observed suite | Reachable; exact-candidate internal verification required |
| Hybrid one-to-one | protocol `1`, suite `0x0002` | ratchet snapshot `1` | Explicit negotiation only; classical fallback is permitted only when the authenticated peer does not advertise hybrid support | Reachable experimental construction; internal verification required |

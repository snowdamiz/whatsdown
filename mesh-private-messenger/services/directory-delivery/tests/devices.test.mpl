...
fn proof() -> Bool!String do
  ...
  Pool.execute(pool, "DROP TRIGGER mesh_test_fail_transparency_append ON transparency_entries", [])?
  ...
  decode_transparency_evidence(encode_transparency_evidence(evidence_for_username(pool,
    "alice",
    1,
    transparency_seed)?)?)?
  ...
  Pool.execute_values(pool,
    ...
    [
      Binary(Crypto.sha256(second.mailbox_token)),
      Binary(repeated(61, 32)),
      Binary(repeated(62, 17))
    ])?
  Pool.execute(pool,
    "CREATE TRIGGER mesh_test_fail_transparency_append BEFORE INSERT ON transparency_entries FOR EACH ROW EXECUTE FUNCTION pg_temp.mesh_test_fail_transparency_append()",
    [])?
...
fn assert_checkpoint_refresh(pool :: PoolHandle, seed :: Bytes) -> Result<(), String> do
  ...
  Pool.execute_values(pool,
    ...
    [
      Text(U64.to_string(stale.timestamp)),
      Binary(stale.signature),
      Text(U64.to_string(stale.sequence))
    ])?

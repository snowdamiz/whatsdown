...
fn bind_on_connection(conn :: borrow PgConn, request :: PushBindRequest) -> PushWrite!String do
  if !(authorized(conn,
    ...
    let changed = Pg.execute_values(conn,
      ...
      [
        Binary(request.mailbox_token_hash),
        Binary(request.wake_token_hash),
        Text(U64.to_string(request.revision)),
        Text(Int.to_string(request.provider)),
        Binary(request.provider_token_ciphertext)
      ])?
    if changed == 1 do
      Ok(PushAccepted)
    else
...
fn unbind_on_connection(conn :: borrow PgConn, request :: PushUnbindRequest) -> PushWrite!String do
  if !(authorized(conn,
    ...
    let changed = Pg.execute_values(conn,
      ...
      [
        Binary(request.mailbox_token_hash),
        Binary(wake_placeholder),
        Text(U64.to_string(request.revision)),
        Binary(token_placeholder)
      ])?
    if changed == 1 do
      Ok(PushAccepted)
    else
